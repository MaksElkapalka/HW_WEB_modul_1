...
class Name(Field):
    ...
    def validate(self, value):        
        while True:
            value = value.title()
            if book.find(value) is None:
                return value
            else:
                print(f'{value} -this name is already taken.')
...
class Record:
    ...
    def __str__(self):
        ...
        emails_str = ', '.join(str(e.value) for e in self.email) if self.email else "None"
...
def add_contact(name=None):
    contact = Record(name) if name else Record(input("Enter the name: "))
    contact.add_phone(input("Enter the phone: "))
    contact.add_birthday(input("Enter the birthday in format dd-mm-yyyy: "))
